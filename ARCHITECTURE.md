...
6. **Bucket CORS** (custom-domain fix, app.cresamor.com — applied and confirmed live 2026-09-03): the browser draws playing video frames to a canvas for reel thumbnails (`home.js`'s `loadReelData`/thumbnail probe) — cross-origin without CORS taints the canvas and silently falls back to a placeholder image (already handled gracefully, but shouldn't be the normal case). The existing R2 API token (Object Read & Write only, step 2 above) cannot read or set bucket CORS itself (`GetBucketCorsCommand` → `AccessDenied`) — this genuinely requires the dashboard, not a token with broader scope minted for the app; the project owner applied it directly. Verified at the HTTP level (not just "policy saved"): a real signed GetObject URL, requested with `Origin: https://app.cresamor.com` and a `Range` header, returns `206 Partial Content` with `Access-Control-Allow-Origin: https://app.cresamor.com` and `Access-Control-Expose-Headers: Content-Range,Content-Length,Accept-Ranges` — exactly the policy below, live. (The in-browser canvas-capture check itself couldn't be completed the same session — this environment's Chrome automation can't decode video at all, confirmed separately, not specific to R2 — but the HTTP-level proof covers the actual CORS mechanism a real browser depends on.) Policy in the Cloudflare dashboard: R2 → the bucket → Settings → CORS Policy:
   ```json
   [
     {
...
**Resend** (2026-09-04) is the first real provider. **Production transport is Resend's HTTPS API** (`https://api.resend.com/emails`, plain `fetch`, no new dependency — Node's runtime already provides global `fetch`), not SMTP. The integration started on Resend's SMTP relay (needed zero structural change to the already-generic-SMTP `email.js` at the time) but two live production sends both hung for roughly a minute before failing — the signature of Render's outbound network being unable to reach Resend's SMTP relay at all (a connection-level block: a bad password fails fast, right after TLS, during AUTH; it doesn't hang). Confirming the correct Resend-documented port (`2465`, implicit TLS) didn't change the outcome, consistent with the network path itself being blocked rather than another port typo. HTTPS on 443 sidesteps this entirely — the same protocol/port every other API call this app makes already uses successfully. Sending domain: `updates.cresamor.com` (verified in Resend, separate from the `cresamor.com` apex Google Workspace already uses for `alex@`/`support@`/`billing@`/`hello@` — keeping transactional volume off the domain real humans' mail reputation depends on).

**Env vars** (Render dashboard only — never local `.env`, never pasted into chat, never committed): `RESEND_API_KEY=<the Resend production API key>` is the real, correctly-named var for the HTTPS transport. `SMTP_HOST`/`SMTP_PORT`/`SMTP_SECURE`/`SMTP_USER` are **legacy — no longer read by any code**, harmless to leave set in Render (nothing depends on removing them). `SMTP_PASS` is a special case: `email.js` still reads it *as a fallback API key* if `RESEND_API_KEY` is unset, specifically so the key already stored in Render under the old SMTP-era var name keeps working without needing re-entry — set `RESEND_API_KEY` explicitly whenever convenient; there's no urgency to migrate off the fallback. `EMAIL_FROM_ADDRESS`/`EMAIL_REPLY_TO` are optional overrides for the two constants below — safe to leave unset.
