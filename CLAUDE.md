...
```
recordingLibrary.js → app.js → mockData.js → messages.js → home.js → recordingPipeline.js → capture.js → teams.js → invitations.js → schedule.js → directMessages.js
```
...
- **`API_URL` in `client/app.js` is an empty string (same-origin), not a hardcoded host.** Custom-domain fix (app.cresamor.com): it used to be hardcoded to the deployed Render URL, which broke the instant a second origin (the custom domain) started serving the same client — every request became cross-origin and Helmet's CSP (no explicit `connect-src`, inheriting `default-src 'self'`) silently blocked it before it ever reached the network. This project's single Express service already serves both the static client and the API from whatever origin request came in on, so same-origin resolution is correct everywhere: the Render domain, the custom domain, and local dev — as long as local dev serves the client from the same local server as the API (the established local-testing pattern: one Express process, `DATABASE_URL` pointed at a disposable local Postgres, `PORT` set locally). No more editing `API_URL` before/after local testing. Full detail in `ARCHITECTURE.md`'s "Deployment reality."
